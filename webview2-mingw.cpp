...
/*
    ...
    LPCWSTR url;
    std::wstring cmdl=TStrToWStr(lpCmdLine);
    if (cmdl.size() > 0) {
      std::filesystem::path p = cmdl;
      cmdl=std::filesystem::absolute(p).wstring();
      url=cmdl.c_str();
    } else url=(LPCWSTR)L"https://wikipedia.org";

		webview2->lpVtbl->Navigate(webview2, url);
		ResizeBrowser(hWnd);
		return S_OK;
	};
